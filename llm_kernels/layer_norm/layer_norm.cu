...
__global__ void layer_norm_v1(T* input, T* output, T* gamma, T* beta, int num_tokens, int hidden_size, float eps) {
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    ...
    mean = cuda_reduce::block_reduce<float, THREADS_PER_BLOCK, cuda_reduce::Add>(mean);
    if (tid == 0) {
    ...
    for (int i = tid; i < hidden_size; i += THREADS_PER_BLOCK) {
        float x = (float)input[bid * hidden_size + i];
        float y = (float)gamma[i] * ((x - s_mean) * s_var) + (float)beta[i];
...
struct vec_pack {
    T data[VEC_SIZE];
    __forceinline__ __device__ float get_sum() {
        float sum = 0.0f;
    ...
    __forceinline__ __device__ float get_diff_square_sum(const float& mean) {
        float sum = 0.0f;
        for (int i = 0; i < VEC_SIZE; ++i) {
    ...
    __forceinline__ __device__ auto do_norm(const vec_pack<T, VEC_SIZE>& gamma, 
                                            const vec_pack<T, VEC_SIZE>& beta, 
                                            const float& mean, 
                                            const float& var) {
        vec_pack<T, VEC_SIZE> norm;
        for (int i = 0; i < VEC_SIZE; ++i) {
            norm.data[i] = (float)gamma.data[i] * ((float)data[i] - mean) * var + (float)beta.data[i];
        }
        return norm;
    }
    ...
    __forceinline__ __device__ auto do_norm_in_place(const vec_pack<T, VEC_SIZE>& gamma, 
                                            ...
                                            const float& var) {
        for (int i = 0; i < VEC_SIZE; ++i) {
            data[i] = (float)gamma.data[i] * ((float)data[i] - mean) * var + (float)beta.data[i];
        }
        return *this;
    }
};
...
template<typename T, int THREADS_PER_BLOCK>
__global__ void layer_norm_v2(T* input, T* output, T* gamma, T* beta, int num_tokens, int hidden_size, float eps) {
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    ...
    for (int i = tid * VEC_SIZE; i < hidden_size; i += THREADS_PER_BLOCK * VEC_SIZE) {
        auto& vec = *(vec_pack_t*)(input + bid * hidden_size + i);
        var += vec.get_diff_square_sum(s_mean);
    }
    ...
    for (int i = tid * VEC_SIZE; i < hidden_size; i += THREADS_PER_BLOCK * VEC_SIZE) {
        ...
        const auto& beta_vec = *(vec_pack<T, VEC_SIZE>*)(beta + i);
        const auto& norm = vec.do_norm(gamma_vec, beta_vec, s_mean, s_var);
...
__global__ void layer_norm_v3(T* input, T* output, T* gamma, T* beta, int num_tokens, int hidden_size, float eps) {
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    constexpr int VEC_SIZE = 16 / sizeof(T);
    ...
    float var = 0.0f;
    for (reg_idx = 0; reg_idx < REG_SIZE; reg_idx++) {
        var += reg[reg_idx].get_diff_square_sum(s_mean);
    }
    ...
    for (int i = tid * VEC_SIZE; i < hidden_size; i += THREADS_PER_BLOCK * VEC_SIZE, reg_idx++) {
        ...
        const auto& beta_vec = *(vec_pack<T, VEC_SIZE>*)(beta + i);
        vec.do_norm_in_place(gamma_vec, beta_vec, s_mean, s_var);
        *(vec_pack<T, VEC_SIZE>*)(output + bid * hidden_size + i) = vec;
    }
}

template<typename T>
void layer_norm_cpu(T* input, T* output, T* gamma, T* beta, int num_tokens, int hidden_size, float eps) {
    for (int i = 0; i < num_tokens; ++i) {
        float mean, var;
        for (int j = 0; j < hidden_size; ++j) {
            mean += (float)input[i * hidden_size + j];
        }
        mean /= hidden_size;
        for (int j = 0; j < hidden_size; ++j) {
            var += ((float)input[i * hidden_size + j] - mean) * ((float)input[i * hidden_size + j] - mean);
        }
        var /= hidden_size;
        for (int j = 0; j < hidden_size; ++j) {
            float res = ((float)input[i * hidden_size + j] - mean) * (1.0f / sqrtf(var + eps)) * (float)gamma[j] + (float)beta[j];
...
int main() {
    ...
    dtype* beta = (dtype*)malloc(sizeof(dtype) * hidden_size);
    ...
    dtype *d_input, *d_output, *d_gamma, *d_beta;
    ...
    cudaMalloc((void**)&d_beta, sizeof(dtype) * hidden_size);
    ...
    cudaMemcpy(d_beta, beta, sizeof(dtype) * hidden_size, cudaMemcpyHostToDevice);
    
    // layer_norm_v1<dtype, THREADS_PER_BLOCK><<<num_tokens, THREADS_PER_BLOCK>>>(d_input, d_output, d_gamma, d_beta, num_tokens, hidden_size, eps);
    // layer_norm_v2<dtype, THREADS_PER_BLOCK><<<num_tokens, THREADS_PER_BLOCK>>>(d_input, d_output, d_gamma, d_beta, num_tokens, hidden_size, eps);
    layer_norm_v3<dtype, THREADS_PER_BLOCK, hidden_size><<<num_tokens, THREADS_PER_BLOCK>>>(d_input, d_output, d_gamma, d_beta, num_tokens, hidden_size, eps);
    ...
    layer_norm_cpu<dtype>(input, output_ref, gamma, beta, num_tokens, hidden_size, eps);

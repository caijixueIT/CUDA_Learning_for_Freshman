...
__global__ void gemm(float* A, float* B, float* C, int M, int N, int K) {
    ...
    // 每个线程负责拷贝 A 中的 BM * BK / numThreadsBlocktile 个元素
    // 这里得到拷贝的起始位置和间隔
    int sRowA = threadIdx.x / BK;
    int sColA = threadIdx.x % BK;
    constexpr int strideA = numThreadsBlocktile / BK;

    // 每个线程负责拷贝 A 中的 BM * BK / numThreadsBlocktile 个元素，从 global memory 到 shared memory
    // 这里得到拷贝的起始位置和间隔
    int sRowB = threadIdx.x / BN;
    int sColB = threadIdx.x % BN;
    constexpr int strideB = numThreadsBlocktile / BN;
    ...
    for (int kidx = 0; kidx < K / BK; kidx++) {
        ...
        for (int i = 0; i < BM; i += strideA) {
            sA[(sRowA + i) * BK + sColA] = A[(sRowA + i) * K + kidx * BK + sColA];
        }
        #pragma unroll
        for (int i = 0; i < BK; i += strideB) {
            sB[(sRowB + i) * BN + sColB] = B[kidx * BK * N + (sRowB + i) * N + sColB];
        }
        __syncthreads();
        for (int k = 0; k < BK; k++) {
            #pragma unroll

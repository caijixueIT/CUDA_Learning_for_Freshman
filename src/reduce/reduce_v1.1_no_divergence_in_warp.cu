...
__global__ void reduce_sum_kernel(float* arr, int n, float* res) {
    ...
    __shared__ float smem[2 * THREAD_PER_BLOCK];

    smem[tid] = arr[idx];
    smem[tid + THREAD_PER_BLOCK] = arr[idx + THREAD_PER_BLOCK];
    __syncthreads();

    for (int i = 1; i <= THREAD_PER_BLOCK; i = i * 2) {
        int x = tid * i * 2;
        if (x < 2 * THREAD_PER_BLOCK) {
            smem[x] += smem[x + i];
        }
        __syncthreads();
        // if (blockDim.x == 0 && tid == 0) {
        //     printf("i = %d, smem[0] = %f\n", i, smem[0]);
        // }
    }
...
int main() {
    ...
    constexpr int threads_per_block = 2 * THREAD_PER_BLOCK;
    ...
    dim3 block_dim(threads_per_block / 2);
